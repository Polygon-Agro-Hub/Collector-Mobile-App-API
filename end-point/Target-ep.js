...
exports.getAllDailyTarget = async (req, res) => {
  ...
  try {
    const { searchText, page, limit } =
      await TargetValidate.getAllDailyTargetSchema.validateAsync(req.query);
    const companyId = req.user.companyId;
...
exports.getTargetsByCompanyId = async (req, res) => {
  try {
    ...
    if (!targets.length) {
      return res
        .status(404)
        .json({ message: "No targets found for this company." });
    }

    const formattedTargets = targets.flatMap((target) => {
      return target.itemId
        ? [
          ...(parseFloat(target.targetA) > 0 ||
            parseFloat(target.todoQtyA) > 0
            ? [
              {
                varietyId: target.varietyId,
                grade: "A",
            ...
            : []),
          ...(parseFloat(target.targetB) > 0 ||
            parseFloat(target.todoQtyB) > 0
            ? [
              {
                varietyId: target.varietyId,
                grade: "B",
            ...
            : []),
          ...(parseFloat(target.targetC) > 0 ||
            parseFloat(target.todoQtyC) > 0
            ? [
              {
                varietyId: target.varietyId,
                grade: "C",
...
exports.getCenterTargetEp = async (req, res) => {
  try {
    ...
    const results = await TargetDAO.getCenterTargetDao(
      centerId,
      varietyId,
      grade,
    );
...
exports.getCenterTarget = async (req, res) => {
  try {
    ...
    if (!centerId) {
      return res
        .status(400)
        .json({ success: false, message: "Missing centerId" });
    }
      ...
      .map((target) => [
        {
          ...
          todo:
            parseFloat(target.qtyA || 0) - parseFloat(target.complteQtyA || 0),
...

        {
          ...
          todo:
            parseFloat(target.qtyB || 0) - parseFloat(target.complteQtyB || 0),
        },

        {
          ...
          todo:
            parseFloat(target.qtyC || 0) - parseFloat(target.complteQtyC || 0),
        },
      ])
      .flat();
...
exports.getTargetForOfficerManagerView = async (req, res) => {
  try {
    ...
    if (!targets.length) {
      return res
        .status(404)
        .json({ message: "No targets found for this officer." });
    }
    ...
    const centerTargets = await Promise.all(
      formattedTargets.map(async (target) => {
        const centerTargetData = await TargetDAO.getCenterTargetDao(
          centerId,
          target.varietyId,
          target.grade,
        );
        return {
          ...
          grade: target.grade,
          centerTarget:
            centerTargetData.length > 0 ? centerTargetData[0] : null,
        };
      }),
    );
