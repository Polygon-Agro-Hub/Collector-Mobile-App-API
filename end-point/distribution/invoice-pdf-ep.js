...
exports.sendSinglePostInvoiceEmail = async (orderIdInput) => {
  try {
    ...
    if (!order || order.message) {
      console.warn(`Order ${orderIdInput} not found for invoice email sending`);
    ...
    if (!isFreeDelivery && !isPickup) {
      ...
      if (cityFromAddress && cityFromAddress.trim() !== "") {
        ...
        if (cityData) {
          ...
          const partialMatch = cities.find((c) => c.city && (c.city.toLowerCase().trim().includes(searchCityName) || searchCityName.includes(c.city.toLowerCase().trim())));
          deliveryFee = partialMatch ? extractFee(partialMatch.charge) : 0;
        }
      }
    }
    ...
    const firstName = customerName.split(" ")[0] || "Valued";
    const lastName = customerName.split(" ").length > 1 ? customerName.split(" ").slice(1).join(" ") : "Customer";
